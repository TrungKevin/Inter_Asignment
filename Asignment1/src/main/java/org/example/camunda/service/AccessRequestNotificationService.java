...
public class AccessRequestNotificationService {
  ...
  public void sendDecisionNotification(Long requestId, boolean approved, String comment) {
    ...
    if (recipient == null) {
      log.warn("Bỏ qua gửi email quyết định vì không có địa chỉ email hợp lệ (requestId={})", requestId);
      return;
    }
  ...
  public void sendNeedInfoNotification(Long requestId, String comment) {
    ...
    String recipient = resolveRecipient(request);//lấy email của user từ request
    if (recipient == null) {
      log.warn("Bỏ qua gửi need-info email vì không có địa chỉ email hợp lệ (requestId={})", requestId);
      return;
    }
  ...
  public void sendCounterProposalNotification(Long requestId, String comment) {
    ...
    String recipient = resolveRecipient(request);//lấy email của user từ request
    if (recipient == null) {
      log.warn("Bỏ qua gửi counter-proposal email vì không có địa chỉ email hợp lệ (requestId={})", requestId);
      return;
    }
  ...
  private String resolveRecipient(AccessRequest request) {
    ...
    String requesterUsername = Objects.toString(request.getRequesterUsername(), "").trim();
    if (requesterUsername.isEmpty()) {
      return null;
    }

    return userRepository.findByUsername(requesterUsername)
        .map(user -> Objects.toString(user.getEmail(), "").trim())
  ...
  private boolean isValidEmail(String value) {
    ...
    try {
      var email = new jakarta.mail.internet.InternetAddress(value.trim(), true);
      email.validate();
      return true;
    } catch (Exception ex) {
      return false;
