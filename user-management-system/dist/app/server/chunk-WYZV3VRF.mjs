...
// src/notifications/webSocket-notification.service.ts
var import_sockjs_client = __toESM(require_entry());
var WebSocketNotificationService = class _WebSocketNotificationService {
  snackBar = inject(MatSnackBar);
  zone = inject(NgZone);
  /** Bắn khi có sự kiện workflow làm thay đổi hàng chờ Camunda — trang task có thể subscribe để reload. */
  camundaTasksRefresh = new Subject();
  camundaTasksRefresh$ = this.camundaTasksRefresh.asObservable();
  ...
  connect() {
    ...
    const client = new Client({
      webSocketFactory: () => new import_sockjs_client.default(`${this.apiUrl}/ws/notifications`, void 0, {
  ...
  scheduleReconnect() {
    ...
    const delayMs = Math.min(3e3 * 2 ** (this.reconnectAttempt - 1), 3e4);
    this.reconnectTimer = setTimeout(() => {
      this.reconnectTimer = null;
      this.connect();
    }, delayMs);
  }
  sendToAdmin(message, requestId, type = "USER_TO_ADMIN") {
  ...
  sendToUser(targetUsername, message, requestId, type = "ADMIN_TO_USER") {
   ...
   */
  shouldReloadCamundaTasks(payload) {
  ...
  static \u0275fac = function WebSocketNotificationService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _WebSocketNotificationService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _WebSocketNotificationService, factory: _WebSocketNotificationService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(WebSocketNotificationService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
*/
//# sourceMappingURL=chunk-WYZV3VRF.mjs.map
